...
class Encoder(chainer.Chain):
    ...
    def forward(self, x, _train=True, _update=True):
        h1 = F.relu(self.bn1(self.conv_1(x), test=not _train, finetune=not _update))
        h2 = F.relu(self.bn2(self.conv_2(h1), test=not _train, finetune=not _update))
        h3 = F.relu(self.bn3(self.conv_3(h2), test=not _train, finetune=not _update))
        h4 = F.relu(self.bn4(self.conv_4(h3), test=not _train, finetune=not _update))
        h5 = F.relu(self.bn5(self.fc5(h4), test=not _train, finetune=not _update))
